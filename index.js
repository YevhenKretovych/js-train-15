...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < characters.length; i++) {
    if (password.length < length) {
      ...
      password += characters.at(indexRandom);
    }
  }
...
function roundObjectValues(obj) {
  ...
  if (typeof obj !== "object" && obj === null) {
    ...
    const obj2 = Object.fromEntries(arrMap);
...
function getFractionalPart(num) {
  ...
  if (typeof num !== "number") {
    ...
    let numFraction = num - numTrunc;

    return Math.fround(numFraction);
  }
}
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (Array.isArray(products) === false) {
    ...
    let totalPrice = Math.fround(
      products.reduce((num, element) => num + element.price, 0)
    );

    return totalPrice === Math.fround(maxPrice);
  }
}
